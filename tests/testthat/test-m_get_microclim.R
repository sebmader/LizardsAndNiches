...

{
  ...
  test_that("With default nyears, the model is run for 1 year with 365 days each with 24 h", {
    ...
    expect_equal(micro$ndays, 365)
    expect_equal(length(micro$metout[,1]), 365 * 24)
  })
  ...
  #   expect_equal(micro$ndays, nyears * 365)
  #   expect_equal(length(micro$metout[,1]), nyears * 365 * 24)
  # })
}
